...
public interface LecturerUpdateRepository extends JpaRepository<LecturerUpdate, UUID> {
    ...
    @Query("SELECT lu FROM LecturerUpdate lu " +
            "JOIN lu.lecturer l " +
            "WHERE lu.status = :status AND l.hidden = false")
    List<LecturerUpdate> findByStatusAndLecturerVisible(@Param("status") PendingStatus status);
